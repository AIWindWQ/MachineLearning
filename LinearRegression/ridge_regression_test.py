...
if __name__ == "__main__":
    # 1、导入测试数据
    print("----------1.load data ------------")
    testData = load_data("./RidgeRegressionData/test_data.txt")
    # 2、导入线性回归模型
    print("----------2.load model ------------")
    w = load_model("./RidgeRegressionTrain/weights")
    # 3、得到预测结果
    print("----------3.get prediction ------------")
    predict = get_prediction(testData, w)
    # 4、保存最终的结果
    print("----------4.save prediction ------------")
    save_result("./RidgeRegressionTrain/predict_result", predict)
