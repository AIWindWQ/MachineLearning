# -*- coding:utf-8 -*-
'''
带有L2正则化项的线性回归模型，正则化项可以平衡模型的方差与偏差
'''
import numpy as np
...
def ridge_regression(feature, label, lam):
    '''
    最小二乘的求解方法
    input:  feature(mat):特征矩阵
            label(mat):标签向量
    output: w(mat):回归系数
    '''
    n = np.shape(feature)[1]
    w = (feature.T * feature + lam * np.mat(np.eye(n))).I * feature.T * label   # 推导出的计算公式
    return w
...
def bfgs(feature, label, lam, maxCycle):
    '''
    利用bfgs训练Ridge Regression模型，其中bfgs是一种拟牛顿算法，用来进行模型参数求解的一种优化算法
    input:  feature(mat):特征矩阵
            label(mat):标签向量
            lam(float):正则化参数
            maxCycle(int):最大迭代次数
...
def lbfgs(feature, label, lam, maxCycle, m=10):
    '''
    利用lbfgs训练Ridge Regression模型，lbfgs是对bfgs的一种改进，相比bfgs，lbsgs更加节约内存
    input:  feature(mat):特征矩阵
            label(mat):标签向量
            lam(float):正则化参数
            maxCycle(int):最大迭代次数
...
def save_weights(file_name, w0):
    ...
    input:  file_name(string):需要保存的文件
            w0(mat):权重
    '''
    f_result = open("./RidgeRegressionTrain/weights", "w")
    m, n = np.shape(w0)
    for i in range(m):
        w_tmp = []
        for j in range(n):
...
if __name__ == "__main__":
    # 1、导入数据
    print("----------1.load data ------------")
    feature, label = load_data("./RidgeRegressionData/train_data.txt")
    # 2、训练模型
    ...
    # 3、保存最终的模型
    print("----------3.save model ------------")
    save_weights("./RidgeRegressionTrain/weights", w0)
