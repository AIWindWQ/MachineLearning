...
import random
"""
SMO算法的第二部分：启发式方法对选择的两个变量进行优化，即选择变量的启发式方法
"""
...
class optStruct:
	...
	def __init__(self, dataMatIn, classLabels, C, toler):
		self.X = dataMatIn								#特征矩阵
		self.labelMat = classLabels						#标签向量
		self.C = C 										#惩罚参数
		self.tol = toler 								#容错率
		self.m = np.shape(dataMatIn)[0] 				#行数，即样本数
		self.alphas = np.mat(np.zeros((self.m,1))) 		#根据样本数，初始化alpha参数为0
		self.b = 0 										#初始化b参数为0
